...
const latestOptimizerLessons = Object.values(OPV_CARDMARKET_OPTIMIZER_LESSONS)
  .filter(lesson => /^optimizer-(?:1[3-9]|[2-9]\d|10[0-4])-critical$/.test(lesson.group))
if (latestOptimizerLessons.length !== 2243) {
  throw new Error(`optimizer 13-104: attese 2243 lezioni uniche, ricevute ${latestOptimizerLessons.length}`)
}
