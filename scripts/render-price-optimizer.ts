...
const main = async () => {
  ...
  for (let page = 0; page < Math.ceil(rows.length / perPage); page += 1) {
    ...
    for (let rowIndex = 0; rowIndex < pageRows.length; rowIndex += 1) {
      ...
      const urls = [row.sourceImageUrl, row.current.imageUrl, row.visual.candidates[0]?.imageUrl]
      const labels = ['OPV', `ATTUALE ${row.current.productId || '-'}`, `PROPOSTA ${row.visual.proposedProductId || '-'}`]
      ...
      composites.push({
        input: textBar(`${row.variantId} - ${row.name}`, 560, 24, '#ffd45a'),
        left: 5,
        top: top + 243,
      })
    }
    ...
    await sharp({
      ...
      .png()
      .toFile(resolve(`.optimizer/group-01-${wantedStatus}-sheet-${page + 1}.png`))
  }
