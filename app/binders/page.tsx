...
import { ArrowLeft, BookOpen, Check, Image as ImageIcon, Pencil, Plus, Save, Search, Share2, Trash2, X } from 'lucide-react'
...
import { BINDER_COLORS, normalizeBinder, normalizeBinderPages, type BinderCard, type BinderRecord } from '@/lib/binders'
...
export default function BindersPage() {
  ...
  const displayedCards = useMemo(() => {
    if (pickerSource === 'catalog') return catalogCards
    const normalized = query.trim().toLowerCase()
    if (!normalized) return collectionCards
    return collectionCards.filter(card => `${card.name} ${card.card_id}`.toLowerCase().includes(normalized))
  }, [catalogCards, collectionCards, pickerSource, query])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: profile }, { data: userCards }] = await Promise.all([
        ...
        supabase.from('user_cards').select('card_id, name, image_url, rarity').eq('user_id', session.user.id).order('name'),
      ])
  ...
  const uploadCover = async (file: File, binderId: string) => {
    const extension = file.name.split('.').pop()?.toLowerCase() || 'jpg'
    const path = `${userId}/${binderId}.${extension}`
    const { error } = await supabase.storage.from('binder-covers').upload(path, file, { upsert: true, contentType: file.type })
    if (error) throw error
    return supabase.storage.from('binder-covers').getPublicUrl(path).data.publicUrl
  }
  ...
  const createBinder = async () => {
    ...
    try {
      ...
      const coverUrl = newCover ? await uploadCover(newCover, id) : null
  ...
  const addPagePair = () => updateActive(binder => {
    const capacity = binder.columns_count * binder.rows_count
    binder.pages.push({ slots: Array(capacity).fill(null) }, { slots: Array(capacity).fill(null) })
    return binder
  })
  ...
  const shareActive = async () => {
    ...
    setSharing(true)
    setStatus('Preparo le due pagine...')
    try {
      if (editing && !(await saveActive())) throw new Error('Prima salva il raccoglitore.')
      const shared = { ...activeBinder, is_shared: true }
      const { error } = await supabase.from('binders').update({ is_shared: true, updated_at: new Date().toISOString() }).eq('id', shared.id).eq('user_id', userId)
      if (error) throw error
      await supabase.from('board_posts').delete().eq('user_id', userId).eq('binder_id', shared.id)
      const { error: postError } = await supabase.from('board_posts').insert({
        user_id: userId,
        type: 'binder',
        title: shared.title,
        message: 'ha creato un raccoglitore. Vai a vederlo e commentalo.',
        binder_id: shared.id,
        card_image_url: shared.cover_image_url,
      })
      if (postError) throw postError
      setActiveBinder(shared)
      setBinders(current => current.map(item => item.id === shared.id ? shared : item))
      setStatus(await shareBinder(shared, spreadIndex, username))
    } catch (error: unknown) {
      if (!(error instanceof DOMException && error.name === 'AbortError')) setStatus(messageFromError(error, 'Condivisione non riuscita.'))
    }
    setSharing(false)
  }
        ...
        {!activeBinder ? (
          <>
            ...
            {!databaseReady ? (
              ...
              <div className="mt-5 grid grid-cols-3 gap-x-3 gap-y-5 sm:grid-cols-4 sm:gap-5 lg:grid-cols-6">
                {binders.map(binder => (
                  <button key={binder.id} type="button" onClick={() => { setActiveBinder(cloneBinder(binder)); setSpreadIndex(0); setEditing(false); setStatus('') }} className="group min-w-0 text-left transition hover:-translate-y-1 active:scale-95">
                    <BinderCover binder={binder} compact className="w-full transition group-hover:shadow-[0_22px_42px_rgba(103,232,249,0.22)]" />
                    <p className="mt-2 truncate px-0.5 text-xs font-black text-white sm:text-sm">{binder.title}</p>
                    <p className="mt-0.5 text-[9px] font-bold text-slate-400">{binder.is_shared ? 'Condiviso' : 'Privato'}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          <>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" onClick={() => setEditing(value => !value)} className={`grid h-10 w-10 place-items-center rounded-2xl border transition active:scale-90 ${editing ? 'border-amber-200/35 bg-amber-300 text-slate-950' : 'border-white/10 bg-slate-950/62 text-white'}`} aria-label="Modifica"><Pencil size={17} /></button>
              <button type="button" onClick={saveActive} disabled={saving} className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-300 text-slate-950 transition active:scale-90 disabled:opacity-50" aria-label="Salva"><Save size={17} /></button>
              <button type="button" onClick={shareActive} disabled={sharing} className="grid h-10 w-10 place-items-center rounded-2xl bg-cyan-300 text-slate-950 transition active:scale-90 disabled:opacity-50" aria-label="Condividi"><Share2 size={17} /></button>
            </div>

            {editing ? (
              <div className="mt-3 flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-slate-950/55 p-2.5">
                <input value={activeBinder.title} onChange={event => updateActive(binder => ({ ...binder, title: event.target.value }))} maxLength={60} className="min-w-[160px] flex-1 rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-base font-bold text-white outline-none focus:border-cyan-300" aria-label="Nome raccoglitore" />
                <div className="flex gap-1.5">
                  {BINDER_COLORS.map(color => <button key={color} type="button" onClick={() => updateActive(binder => ({ ...binder, cover_color: color }))} className={`h-8 w-8 rounded-full border-2 transition active:scale-90 ${activeBinder.cover_color === color ? 'border-white scale-110' : 'border-transparent'}`} style={{ backgroundColor: color }} aria-label={`Copertina ${color}`} />)}
                </div>
                <div className="flex rounded-xl border border-white/10 bg-slate-900 p-1">
                  {layouts.map(layout => <button key={layout.label} type="button" onClick={() => changeLayout(layout.columns, layout.rows)} className={`rounded-lg px-2 py-1.5 text-xs font-black transition ${activeBinder.columns_count === layout.columns && activeBinder.rows_count === layout.rows ? 'bg-cyan-300 text-slate-950' : 'text-slate-300'}`}>{layout.label}</button>)}
                </div>
                <button type="button" onClick={addPagePair} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-cyan-200/25 bg-cyan-300/10 px-3 text-xs font-black text-cyan-100 transition active:scale-95"><Plus size={14} /> 2 pagine</button>
                <button type="button" onClick={deleteActive} className="grid h-9 w-9 place-items-center rounded-xl border border-rose-200/20 bg-rose-400/10 text-rose-100 transition active:scale-90" aria-label="Elimina raccoglitore"><Trash2 size={15} /></button>
              </div>
            ) : null}
            ...
            <div className="mt-2 overflow-x-hidden">
              <BinderBook binder={activeBinder} spreadIndex={spreadIndex} onSpreadChange={setSpreadIndex} editable={editing} onSelectSlot={(page, slot) => setPickerSlot({ page, slot })} onRemoveCard={removeCard} />
            </div>
      ...
      {introOpen ? (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/72 p-4 backdrop-blur-md">
          <div className="w-full max-w-md rounded-[1.75rem] border border-cyan-200/20 bg-[#173c46] p-5 shadow-2xl">
            ...
            <h2 className="mt-4 text-2xl font-black">I tuoi raccoglitori</h2>
            <div className="mt-4 space-y-3 text-sm leading-6 text-slate-200">
              <p><strong>1.</strong> Scegli copertina e formato delle tasche.</p>
              <p><strong>2.</strong> Apri il raccoglitore e inserisci le carte pagina per pagina.</p>
              <p><strong>3.</strong> Condividi una doppia pagina con logo OPV, like e commenti.</p>
            </div>
      ...
      {createOpen ? (
        <div className="fixed inset-0 z-[95] flex items-end justify-center bg-black/72 p-2 backdrop-blur-md sm:items-center sm:p-4" onClick={() => setCreateOpen(false)}>
          <div className="w-full max-w-xl rounded-[1.75rem] border border-white/12 bg-[#173c46] p-4 shadow-2xl sm:p-5" onClick={event => event.stopPropagation()}>
            ...
            <input ref={coverInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={event => setNewCover(event.target.files?.[0] || null)} />
            <button type="button" onClick={() => coverInputRef.current?.click()} className="mt-3 inline-flex items-center gap-2 rounded-xl border border-white/12 bg-white/[0.06] px-3 py-2 text-xs font-black text-slate-200 transition active:scale-95"><ImageIcon size={15} /> {newCover ? newCover.name : 'Immagine opzionale'}</button>
          ...
          <div className="flex max-h-[88dvh] w-full max-w-3xl flex-col overflow-hidden rounded-[1.75rem] border border-white/12 bg-[#163943] shadow-2xl" onClick={event => event.stopPropagation()}>
            <div className="flex items-center gap-2 border-b border-white/10 p-3">
              <div className="flex rounded-xl border border-white/10 bg-slate-950/40 p-1">
                <button type="button" onClick={() => setPickerSource('collection')} className={`rounded-lg px-3 py-2 text-xs font-black ${pickerSource === 'collection' ? 'bg-cyan-300 text-slate-950' : 'text-slate-300'}`}>Mie carte</button>
                <button type="button" onClick={() => setPickerSource('catalog')} className={`rounded-lg px-3 py-2 text-xs font-black ${pickerSource === 'catalog' ? 'bg-cyan-300 text-slate-950' : 'text-slate-300'}`}>Catalogo</button>
              </div>
